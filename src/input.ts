import { RectObject, PathObject, GroupObject, Scene } from './scene';
...
export class InputManager {
    ...
    async handleSVGDrop(file: File) {
        ...
        paths.forEach(p => {
            ...
            if (d) {
                const ckPath = (this.scene.ck as any).Path.MakeFromSVGString(d);
                if (ckPath) {
    ...
    handleSmartPaint(pos: { x: number; y: number }) {
    ...
    handleSelection(pos: { x: number; y: number }) {
    ...
    onMouseMove(e: MouseEvent) {
        ...
        if (this.ui.activeTool === 'selection' && this.scene.selection.length > 0) {
            ...
            const sel = this.scene.selection[0] as any;
            sel.x = (sel.x || 0) + dx;
            sel.y = (sel.y || 0) + dy;
